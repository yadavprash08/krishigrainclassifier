    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if (arguments.isAddToStore()) {
                ...
                System.out.println(MessageFormat.format("Processing the image as {0} grains of type {1}",
                    particleType, grainType));

                ImageProcessor processor = injector.getInstance(ImageProcessor.class);
                processor.processImage(arguments.getImageToProcess())
                    .stream()
                    .map(a->a.withParticleType(particleType.getStringValue()))
                    .map(a->a.withGrainType(grainType.getStringValue()))
